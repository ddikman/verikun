...
- **Keep the docs contract — in these two cases only.** Update `README.md`, `.claude/skills/verikun/SKILL.md` **and** the owning `docs/` page (the *Which page owns which contract* table in `CLAUDE.md`), then tick the matching box in the PR's *Docs & contracts* section, when either:
  - **a contract moved or arrived** — a new or changed command, flag, default, exit code, report format, `device set` key or platform capability; or
  - **a doc is now false** — a sentence still on a page is no longer true. Delete the stale clause rather than restate it.

  Otherwise — an ordinary bug fix, where behaviour now matches what the docs already say — it is **changelog only**; write nothing. See `CLAUDE.md`, *Versioning, changelog & docs*.
...
Fill the sections of [`.github/pull_request_template.md`](../../../.github/pull_request_template.md), **deleting any that don't apply**. Optional shorter headings — **rename only, in template order, never merge sections** (TL;DR and Docs & contracts keep their names):

| Template heading | Shorter alias |
|---|---|
| What changed? | How it works |
| Why make this change? | Why |
| How to test? | Verified |
| Out of scope & next steps | Deliberately not done |

`How it works` is the design section only — do **not** rename `How to test?` to it. Design and verification stay two sections however they're titled. It's a checklist of what a reviewer needs, not a form.
...
- **How to test?** — concrete reviewer steps. Skip anything CI already proves (don't write "builds clean" / "units pass"). **Show the result, not just what you ran:** a before/after, trimmed command/test output, a screenshot, or a reference — not a bare "tested it". Beyond a small fix, make it **a table, one row per claim**, each row something the reviewer could go and check; that scans better than prose. When the table covers unit and on-device checks, **delete** the template's `#### New tests` and `#### Manual testing performed` sub-headings — the table replaces them. For a small fix, a single prose line under `How to test?` is enough.
  - **New tests** *(prose only — skip when using a table)* — the flows/edge cases now covered in `tests/*.test.ts` (the unit suite covers the platform-agnostic core), plus the total, **not** a list of files. If none, say why (e.g. "driver/platform method — device-verified only", or "covered by existing `selector.test.ts`").
  - **Manual testing performed** *(prose only — skip when using a table)* — there is no device CI, so this is the only record of on-device behaviour: device or emulator + OS, and what you confirmed — claims and outcomes, not a transcript of the session. The connected device may be a personal phone — avoid destructive actions (submitting forms, creating accounts) while exercising it.
- **Docs & contracts** — tick the boxes that apply (README + SKILL + the owning `docs/` page when a contract moved or a doc went false; `usageText()` / `RECORDABLE` for a new command; a `tests/<module>.test.ts` case for a new pure core function). Delete the block if the PR touches none of it — an ordinary bug fix ticks nothing here.
...
Good — plain intent, a named idea instead of files, new flags called out:

> **TL;DR** New `vk log` command pulls the device's logs into the test-run report, scoped to the current run by default.
>
> **What changed?**
>
> **Log capture is on demand, and scoped to the run.** A device's log buffer is a ring that long outlives any one run, so pulling all of it would bury the failure in lines from before the test started.
>
> **Window flags default to the current session.** New `--since` / `-n` / `--full` pick the slice; the default excludes logs from before the run started.
