...
export function buildServer(config: ServerConfig): Server {
   ...
   */
  const pickFailoverDevice = async (failed: string): Promise<string | null> => {
   ...
   *
   * On a pool the lease dies with the device rather than following it: `reapLeases` drops
   * any lease whose serial has left, so the run that hit the failure gets a fresh device
   * on its next request and every other lease is untouched throughout.
   *
