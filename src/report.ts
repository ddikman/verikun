...
export function toSuiteHtml(suite: SuiteRun, opts: { linkBase?: string } = {}): string {
  ...
  // The Device COLUMN is a separate question from parallelism: a pooled server the suite
  // sized to one lane, or `--concurrency 1` over two phones, still ran on a device the
  // header cannot name. Show it whenever a row knows something `SuiteRun.device` does
  // not — which is never true of a serial run, where the header already says it.
  const showDevice = suite.tests.some((x) => x.device && x.device !== suite.device);
...
${abortedBanner}${warningsBanner}  <table class="tests">
    <thead><tr><th></th><th>Test</th>${showDevice ? '<th>Device</th>' : ''}<th>Steps</th><th>Repairs</th><th>Cost</th><th>Duration</th></tr></thead>
    <tbody>
${suite.tests.map((x) => suiteTestRow(x, linkBase, showDevice)).join('\n')}
    </tbody>
  </table>
</div>
</body>
