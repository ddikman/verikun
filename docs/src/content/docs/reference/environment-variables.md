...
| `VERIKUN_NO_ADB_RECYCLE` | unset | Set to `1` to stop [`vk server`](/verikun/guides/remote-devices-and-ci/) restarting a rotted adb server while idle. Only needed when something else on the host uses `adb` alongside the server. Android hosts only; the check is macOS-only. |
