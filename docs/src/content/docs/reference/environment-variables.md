...
| `VERIKUN_COMPANION` | on | The [Android companion](/verikun/guides/companion/) is used by default; `0` (or `false`/`off`/`no`) turns it off. It makes hierarchy reads ~0.2s instead of ~2.4s, at the cost of holding the device's single `UiAutomation` connection while it runs. |
